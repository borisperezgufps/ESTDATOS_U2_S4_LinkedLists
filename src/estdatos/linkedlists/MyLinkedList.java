...
public class MyLinkedList<E> {
	...
	/**
	 * Este metodo se asume que hace parte de una lista ordenada.
	 * con cuatro elementos de tipo entero.
	 * @param element
	 */
	public void insertarNodo(Node<E> nodoAnterior, Node<E> nodo, Node<E> nodoNuevo) {
		Node<E> current = null;
		if(nodo==null)		
			current = head;
		else 
			current = nodo;
		if(!nodoNuevo.esMenor(current)) {
			insertarNodo(current, current.getNext(), nodoNuevo);
		}else {
			nodoAnterior.setNext(nodoNuevo);
			nodoNuevo.setNext(current);
		}
		size++;
	}
	...
	private void imprimirRecursivo(Node<E> apuntador) {
		
		if(apuntador!=null) {
			System.out.println(apuntador.getData());
			imprimirRecursivo(apuntador.getNext());
		}
	}
	
	public void imprimirRecursivo(Node<E> apuntador, boolean esPrimeraVez) {
		...
		if(apuntador!=null) {
			System.out.println(apuntador.getData());
			imprimirRecursivo(apuntador.getNext(), false);
